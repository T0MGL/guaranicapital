...
/* React escapes the text child of a <style> tag when it renders on the
   server, and this block is the one on the page that contains an apostrophe:
   `content: '';` came back as `content: &#x27;&#x27;;`. Entities are not
   decoded inside <style>, so the CSS parser dropped the declaration, the
   footer hairline never drew, and the text mismatch made React throw the
   whole prerendered tree away and rebuild it on the client. Handing the CSS
   over as raw HTML is the only way React emits it verbatim. The object is
   module scope so it is not rebuilt on every render.
   scripts/prerender.mjs fails the build if any served <style> carries an
   entity again. */
const FOOTER_CSS = {
  __html: `
        .footer {
...
export const Footer = () => {
  ...
  return (
    <footer className="footer">
      ...
      <style dangerouslySetInnerHTML={FOOTER_CSS} />
    </footer>
  );
};
